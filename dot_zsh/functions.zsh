...
function gog() {
  gum spinner --title "Fetching $1..." -- ghq get git.sonr.io/$1
  gum spinner --title "Adding $1..." -- zoxide add $GHQ_ROOT/git.sonr.io/$1
}

function ghg() {
  ghq get $1
  zoxide add $GHQ_ROOT/github.com/$1
}
...
# Set GHQ_ROOT to the root of your project
# On macOS: /Volumes/home/Code if it exists, otherwise ~/Developer
# On Linux: ~/code
...
function dxr() {
  # If a command argument is provided, execute it directly
  if [ -n "$1" ]; then
    devbox run "$1"
    return
  fi
  ...
  if [ ! -f devbox.json ]; then
    echo "Error: devbox.json not found in the current directory."
    return 1
  fi

  # Use jq to extract the script keys, then fzf to prompt user for a selection
  local selected_command
  selected_command=$(tv devbox)
  ...
  # Run the selected command using devbox run
  devbox run "$selected_command"
}
...
function url_preview() {
  ...
  local url="$1"
  local cache_dir="${XDG_CACHE_HOME:-$HOME/.cache}/url_preview"
...
[[ -f /Users/prad/.config/.dart-cli-completion/zsh-config.zsh ]] && . /Users/prad/.config/.dart-cli-completion/zsh-config.zsh || true
## [/Completion]
...
function ghdocdl() {
  ...
  local dest_dir="$HOME/Downloads/$repo-docs"
  mkdir -p "$dest_dir"
...
function go-nuke() {
  echo "🚨 WARNING: This will clean your Go module cache and $HOME/go directory 🚨"
  echo "Press Ctrl+C to abort or Enter to continue..."
  read
  ...
  echo "📦 Removing downloaded modules in $HOME/go/pkg/mod..."
  rm -rf $HOME/go/pkg/mod/* 2>/dev/null || echo "No modules to remove."

  echo "🔨 Removing compiled binaries in $HOME/go/bin..."
  rm -rf $HOME/go/bin/* 2>/dev/null || echo "No binaries to remove."

  echo "🧪 Removing test results..."
  find $HOME/go -name "*.test" -type f -delete

  echo "🗄️ Removing temporary build files..."
  find $HOME/go -name "*.o" -type f -delete
  find $HOME/go -name "*.a" -type f -delete
  find $HOME/go -name "*.so" -type f -delete

  echo "✅ Go environment has been nuked! Your $HOME/go directory is clean."
  echo "👉 Note: Your source code in $HOME/go/src remains untouched."
}
...
function ghn() {
  ORG_NAME=$(gum choose --header "Select parent org:" "sonr-io" "prnk28")
  REPO_NAME=$(gum input --header "Enter a name for your new repo")
  if [ -z "$REPO_NAME" ]; then
    gum log --level error "Please enter a valid name for the repo and organization."
    return 1
  ...
  cd ~/Developer/github.com/$ORG_NAME/$REPO_NAME
}
